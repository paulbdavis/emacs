...
Return a list of containers names"
  (cl-loop for line in (cdr (apply #'process-lines kubernetes-tramp-kubectl-executable (list "get" "--all-namespaces" "po" )))
           for info = (split-string line "[[:space:]]+" t)
           collect (format "%s@%s" (car info) (car (cdr info)))))
...
(defun kubernetes-tramp-add-method ()
  ...
  (add-to-list 'tramp-methods
               `(,kubernetes-tramp-method
                 ...
                 (tramp-login-args         (,kubernetes-tramp-kubectl-options ("-n" "%u") ("exec" "-it") ("%h") ("sh")))
